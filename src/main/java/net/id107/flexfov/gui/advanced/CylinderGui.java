...
public class CylinderGui extends AdvancedGui {
	...
	protected void init() {
		...
		SimpleOption<Double> FOVX = new SimpleOption<Double>("cylinderFovX", 0, 360, 1,
				(gameOptions) -> {return Projection.getProjection().getFovX();},
				(gameOptions, number) -> {Projection.fov = number; ConfigManager.saveConfig();},
				(gameOptions, SimpleOption<double>) -> {return Text.literal("Horizontal FOV: " + Math.round(Projection.getProjection().getFovX()));});
		addDrawableChild(FOVX.createWidget(client.options, width / 2 - 180, height / 6 + 60, 360));
		
		SimpleOption<Double> FOVY = new SimpleOption<Double>("cylinderFovY", 0, 180, 1,
				(gameOptions) -> {return Projection.getProjection().getFovY();},
				(gameOptions, number) -> {Cylinder.fovy = number; ConfigManager.saveConfig();},
				(gameOptions, SimpleOption<Double>) -> {return Text.literal("Vertical FOV: " + Math.round(Projection.getProjection().getFovY()));});
